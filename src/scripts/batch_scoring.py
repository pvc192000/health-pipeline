#!/usr/bin/env python3
"""Batch ML scoring — trains 4 wellness models and writes predictions to S3 curated bucket."""
import pandas as pd
import numpy as np
...
def compute_targets(df):
    """Compute composite wellness scores as training targets."""
    df["sleep_quality_score"] = np.clip(
        0.4 * np.clip(df["sleep_duration"].fillna(0) / 480000 * 100, 0, 100)  # ms to min, 8hr=100
        + 0.3 * np.clip(df["sleep_efficiency"].fillna(0) * 100, 0, 100)
        + 0.3 * np.clip(df["sleep_deep_ratio"].fillna(0) * 100, 0, 100),
        0, 100,
    )
    df["stress_recovery_score"] = np.clip(
        ...
        + 0.3 * np.clip(100 - df["stress_score"].fillna(50), 0, 100),
        0, 100,
    )
    df["activity_strain_score"] = np.clip(
        0.5 * np.clip(df["steps"].fillna(0) / 10000 * 100, 0, 100)
        + 0.5 * np.clip(
            (df["lightly_active_minutes"].fillna(0) + df["moderately_active_minutes"].fillna(0)
             + df["very_active_minutes"].fillna(0)) / 60 * 100, 0, 100),
        0, 100,
    )
    df["illness_risk_score"] = np.clip(
        ...
        + 0.3 * np.clip((df["resting_hr"].fillna(60) - 60) / 20 * 100, 0, 100),
        0, 100,
    )
    return df
...
def main():
    ...
    # Download all parquet files from daily_summaries
    ...
    score_df = df_model[output_cols].copy()
    # Include participant_id and date for joining
